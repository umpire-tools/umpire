...
See [Selection](/concepts/selection/) for the full breakdown of patterns.
...
- Solid has first-class shared-store support in [`@umpire/solid`](/adapters/solid/#fromsolidstore) via `fromSolidStore()`, which uses `@umpire/signals` under the hood.
