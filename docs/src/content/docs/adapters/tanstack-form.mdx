...
Example — a `state` field that's only available when `country` is `'US'`, and a `vatId` field that must pass a fairness check:
...
const engine = umpire({
  ...
  rules: [
    enabledWhen('state', (v) => v.country === 'US', { reason: 'Only US addresses have a state' }),
    fairWhen('vatId', (v) => /^[A-Z]{2}/.test(String(v.vatId)), { reason: 'VAT ID must start with two letters' }),
  ],
})
...
const vatIdValidators = umpireFieldValidator(engine, 'vatId', {
  events: ['onChange', 'onBlur'],
})
// → { onChange: fn, onChangeListenTo: [], onBlur: fn, onBlurListenTo: [] }
```
...
// allValidators.vatId  → { onChange: fn, onChangeListenTo: [] }
...
function AddressForm() {
  ...
  return (
    <form>
      ...
      {umpireForm.field('vatId').enabled && (
