...
The TC39 adapter is useful for environments targeting the eventual native signal API, or for server-side availability checks where fouls tracking isn't needed. For full functionality including fouls, use the [Preact](/umpire/adapters/signals/preact/) or [alien-signals](/umpire/adapters/signals/alien/) adapter instead.
