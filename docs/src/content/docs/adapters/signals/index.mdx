...
[`useUmpire`](/adapters/react/) derives everything on every render — `check()` runs, the full availability map rebuilds, React reconciles the whole tree. For most forms that's fine.
...
Solid also has [`@umpire/solid`](/adapters/solid/) if you'd rather start with `useUmpire()` or `fromSolidStore()` instead of `reactiveUmp()`.
