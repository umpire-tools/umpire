...
const hintConditions = {
  cpuBrand: scorecard.fields.cpu.facts?.brand,
...
That feels like a better extraction seam than the earlier history-flags helper. The reusable part is not rendering. It is “compose umpire state with domain facts, then let hints consume that richer scorecard.” React can wrap the hint runtime with a hook, but hint selection itself stays derived instead of syncing through an effect.
...
That filtering logic is still not what Umpire is responsible for. The component keeps the product catalog in userland, but it now declares named build facts once and reuses them from `fairWhen()`, the scorecard, and render. Umpire owns the rule result and the cascade. The app still owns the catalog.
