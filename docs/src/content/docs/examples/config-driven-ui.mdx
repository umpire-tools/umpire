...
- [Quick Start](/learn/) &mdash; ten-minute walk through the rule primitives.
- [`@umpire/json`](/extensions/json/) &mdash; the portable schema in full, including `excluded` and `conditions`.
- [@umpire/dsl](/extensions/dsl/) &mdash; the pure `expr.*` vocabulary and compiler helpers.
- [Builders & Checks](/extensions/json/builders/) &mdash; JSON-specific `expr.check()`, portable validators, and rule builders.
- [Availability vs Validation](/concepts/availability/) &mdash; the mental model behind `enabled`, `required`, `fair`.
- [PC Builder](/examples/pc-builder/) &mdash; a cascade-heavy example in the same declarative spirit.
