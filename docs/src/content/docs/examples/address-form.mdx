...
| **custom fair rule** | `postalCode` format validation switches based on country, and declares `country` as a graph source so validators re-run when country changes. |
...
The `useUmpireForm` hook subscribes to the form store, derives availability and fouls, and applies strike-on-transition automatically. Conditional rendering gates on `umpireForm.field('state').enabled` and `umpireForm.field('province').enabled` — no manual visibility logic needed.
