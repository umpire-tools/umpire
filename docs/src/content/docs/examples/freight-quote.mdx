...
| **`disables`** | _(not used here — see the [Printer Menu](/adapters/zustand/) for `disables` in action)_ |
...
This demo uses [`@umpire/signals`](/adapters/signals/) with [Preact](https://preactjs.com/) and [`@preact/signals`](https://preactjs.com/guide/v10/signals/). Each field component reads signal-backed values during render — Preact auto-subscribes and re-renders only the fields that changed.
