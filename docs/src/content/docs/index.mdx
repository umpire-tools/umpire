...
Use `@umpire/core` when you just need pure availability logic. Add `@umpire/react`, `@umpire/signals`, `@umpire/store`, or a store-specific entry point like `@umpire/zustand`, `@umpire/redux`, or `@umpire/tanstack-store` when you want an adapter on top.
