...
## Could you just use if statements?

Yes. Until you can't.

The moment fields start depending on each other — disabling when conditions change, requiring cleanup when values go stale, branching in more than one direction at once — the ifs multiply faster than they stay readable. Umpire gives that logic a single declarative home, with a queryable result you can act on anywhere.
