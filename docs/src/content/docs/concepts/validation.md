...
For the common case — building a schema that reflects current availability and filtering errors to active fields — [`@umpire/zod`](/adapters/validation/zod/) handles the wiring. This page covers the conceptual boundary and the `check()` bridge that makes composition work regardless of which library you use.
...
This is exactly what `deriveSchema` and `deriveErrors` do. See [Validator Integrations](/adapters/validation/) for the general pattern and how it extends to other libraries.
...
- [Validator Integrations](/adapters/validation/) — the integration contract and how it applies to any library
- [`@umpire/zod`](/adapters/validation/zod/) — first-class Zod integration with `deriveSchema` and `deriveErrors`
- [Satisfaction semantics](/concepts/satisfaction/) — how Umpire defines "present"
- [`check()` in the rules API](/api/rules/check/) — full signature and validator shapes
